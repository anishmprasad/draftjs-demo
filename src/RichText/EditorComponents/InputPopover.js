...
export default class InputPopover extends Component {
	...
	render() {
		let { props } = this;
		let className = cx(props.className, 'input-popover');
		return (
			<div className={className}>
				<div className={'inner'}>
					...
					<ButtonGroup className={'buttonGroup'}>
						<IconButton label='Cancel' iconName='cancel' onClick={props.onCancel} />
						<IconButton label='Submit' iconName='accept' onClick={this._onSubmit} />
					</ButtonGroup>
				</div>
			</div>
		);
